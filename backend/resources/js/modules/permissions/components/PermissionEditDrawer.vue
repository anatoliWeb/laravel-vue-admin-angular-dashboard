<template>
  <BaseForm :model="form.model" @submit="handleSubmit">
    <BaseFormSection title="Edit permission">
...
const props = defineProps<{ closeDrawer?: () => void; permission: PermissionListItem; onUpdated?: (item: PermissionListItem) => void }>();
...
const handleSubmit = async ({ model }: FormSubmitContext<Record<string, unknown>>): Promise<void> => {
  ...
  if (result) { toast.success({ title: 'Permission updated', message: 'Permission edit shell completed.' }); close(); }
};
</script>
