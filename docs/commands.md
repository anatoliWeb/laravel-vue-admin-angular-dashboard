...
```
docker exec -it saas_mysql mysql -u root -p
```

### Backup database
```
docker exec saas_mysql mysqldump -u root -p saas > backup.sql
```

### Restore database
```
cat backup.sql | docker exec -i saas_mysql mysql -u root -p saas
```
...
### Enter Redis CLI
```
docker exec -it saas_redis redis-cli
```
